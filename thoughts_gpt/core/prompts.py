...
summarization_template = """使用下面提供的文档摘录（排名不分先后）作为来源，将这些内容提炼成主要主题的最终综合摘要并推理出{suggested_questions_limit}个联想问题。始终根据文档的摘录推理个{suggested_questions_limit}联想问题。你应该始终分析并推理出答案背后的{suggested_questions_limit}个相关的联想问题问题。始终在您的答案中包含“联想问题”部分
...
def get_summarization_prompt(suggested_questions_limit=5):
...
def get_prompt(qtype: str = "qa", template: str=default_template, **kwargs) -> PromptTemplate:
    if qtype == "qa":
